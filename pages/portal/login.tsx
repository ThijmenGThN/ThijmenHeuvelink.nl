...
export default function Menu({ API }: { API: Directus }) {
  ...
  useEffect(() => {
    API.users.me.read().then(() => window.location.replace('/portal'))
  }, [])
  ...
  return (
    <>
      ...
      <div className='min-h-screen w-1/2 min-w-[256px] mx-auto flex justify-center items-center flex-col font-semibold gap-2'>
        <p className='text-3xl font-bold mb-8'>Portal</p>
        <div className='w-full group'>
          <p className='group-hover:italic text-sm'>E-mail</p>
          <input onChange={({ target }) => setEmail(target.value)} className='bg-neutral-100 px-4 py-2 w-full rounded-lg group-hover:italic' placeholder='user@heuve.link' type="text" />
        </div>
        <div className='w-full group'>
          <p className='group-hover:italic text-sm'>Password</p>
          <input onChange={({ target }) => setPassword(target.value)} className='bg-neutral-100 px-4 py-2 w-full rounded-lg group-hover:italic' placeholder='••••••••' type="password" />
        </div>
